...
FEDIVERSE_INSTANCE_URL = 'https://your.instance.social'  # e.g., mastodon.social, fosstodon.org
ACCESS_TOKEN = 'your_access_token_here'  # Generate in your Fediverse account settings
...
def format_post(publication):
    ...
    year = publication['first published']
    collections = publication['collections'].strip()
    moods = publication['moods'].strip()
    themes = publication['themes'].strip()
    isbn = publication['isbn'].strip() if publication['isbn'].strip() else "no ISBN"

    post_text = f"""A Poem of the Day from Landroverfarm Press
